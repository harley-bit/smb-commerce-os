...
|  11 | Platform admin console                 |      21 | **G11** — support actions are grant-scoped, time-boxed, audit-logged |
|     | **Total**                              | **197** |                                                                      |

**Phase 11 was added 2026-09-08**, after the founder requested a scoped platform-admin/power-user surface (see `16_platform_admin_console.md`). It is placed after Phase 10 specifically so it doesn't renumber any already-built card — it does not gate `G7` or `G10` in the sense that a pilot could launch without it, but Epic A within it (`D177`–`D182`, break-glass access) should land before the pilot has been live long enough to need a real support incident handled. Full rationale, roles, architecture, and workflows: `16`.
...
**Phase 3 — Availability engine and services (18 days).** Resources, availability rules and exceptions, the interval reservation model, conflict detection, and service booking. _Gate G3 requires a concurrency test that fires simultaneous bookings at the last slot and proves exactly one succeeds._
...
**Phase 5 — Payments (14 days).** Stripe Connect onboarding, split payments, authorization holds with capture and release, refunds per mode, and webhook handling with idempotency. _Card data never touches our servers — hosted fields only._
...
**Phase 8 — Mobile application (16 days).** Customer flows on iOS and Android from the shared codebase. _Candidate for deferral until after the pilot — see decision D3._
...
**Phase 11 — Platform admin console (21 days).** A separate `apps/admin` app for platform staff (`PLATFORM_SUPPORT | PLATFORM_ADMIN | PLATFORM_SUPERADMIN`) — break-glass merchant access with mandatory reason and time-boxed grants, support actions (void/refund, release a stuck reservation, notification resend), merchant lifecycle (suspend, dual-control reinstate, onboard/offboard), platform-wide aggregate reporting, feature flags, and platform staff role management. Full detail: `16`. _Gate G11 requires a test proving no merchant-data access succeeds without an active, unexpired grant, and that every support action produces an audit row carrying the grant ID._
