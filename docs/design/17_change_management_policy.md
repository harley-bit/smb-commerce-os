...
**Also worth noting:** two real branch/worktree collisions happened on 2026-09-08 — the automated loop and an interactive session share one working directory, so committing to whatever branch happens to be checked out at the moment the loop fires. Twice, an interactive session had checked out a feature branch for unrelated PR work, and the loop's next card landed there instead of on `main`. Both times the work was recovered by cherry-picking onto a fresh branch off `main` — nothing was lost, but this is a real hazard worth designing around (a dedicated git worktree for the loop, or pausing the launchd schedule before switching branches interactively) rather than continuing to rely on catching it after the fact.
