...
**New scope (added 2026-09-08)**

16. `16_platform_admin_console.md` — platform-staff/power-user UI and workflow: break-glass support access, merchant lifecycle, reporting, feature flags, platform role management. Placed as Phase 11 (`D177`–`D197`), after `10` so it doesn't renumber any built card. Amends `03` and `07`.
17. `17_change_management_policy.md` — the actual git/PR/branch-protection process this repo runs, grounded in real settings and file paths (not a forward plan). Amends `10`'s branching section; is the evidence artifact `09`'s change-management control row points to.
